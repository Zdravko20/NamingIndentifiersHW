...
namespace Orders
{
    class OrdersMain
    {
        ...
        private static string MostProfitableCategory(IEnumerable<Product> products,IEnumerable<Order> orders,IEnumerable<Category> categories)
        {
            var mostProfitableCategory = orders
                .GroupBy(o => o.ProductId)
                .Select(g => new { CatId = products.First(p => p.Id == g.Key).CategoryId,
                                   Price = products.First(p => p.Id == g.Key).UnitPrice, Quantity = g.Sum(p => p.Quantity) })
                .GroupBy(gg => gg.CatId)
                .Select(grp => new { CategoryName = categories.First(c => c.Id == grp.Key).Name,
        ...
        private static string TopProducts(IEnumerable<Product> products,IEnumerable<Order> orders, int count)
        {
            ...
            var topProducts = orders
                .GroupBy(o => o.ProductId)
                .Select(grp => new { Product = products.First(p => p.Id == grp.Key).Name,
